...
The `calc()`/`min()` declarations at the bottom of that diagram — the derived layer spelled out under [The Computation](#the-computation) — are generated by `src/radius/emitDerived.ts` and appended to `dist/css/tokens.css` once, in `:root`. They are never hand-written, and they are not repeated per mode: because custom properties are substituted per element at computed-value time, one emission recomputes correctly inside every `[data-radius-mode]` subtree.
...
src/
  radius/
    ...
    emitDerived.ts                       # The derived CSS layer, emitted once
  tokens/
